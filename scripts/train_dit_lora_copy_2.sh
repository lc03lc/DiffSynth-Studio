...
LOGDIR="/raid/workspace1/lc/HumanAndRobot/Logs/wan_lora_dit_gpu6"
...
for category in "${CATS[@]}"; do
  ...
  if [[ ! -f "$META_CSV" ]]; then
    echo "[GPU6][SKIP] ${category}: ${META_CSV} 不存在" | tee -a "$LOGDIR/${category}.log"
    continue
  fi

  echo "[GPU6][RUN] ${category} -> ${OUT}"
